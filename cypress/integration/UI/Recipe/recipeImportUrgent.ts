...
When('eu realizo a importação de uma receita urgentes', () => {
    cy.importRecipe({
        ...
        assert.exists(result.success, result.error);
        cy.wrap(result.urgentRecipe).as('urgentRecipe');
    });
});

Then('os dados das receitas importadas urgentes, devem ser capturados e exibidos corretamente', () => {

    cy.get('@urgentRecipe').then((urgentRecipe) => {
        expect(urgentRecipe).to.exist; // Verifica se `urgentRecipe` existe
        cy.log(`Receita urgente importada: ${urgentRecipe ? 'Marcado' : 'Desmarcado'}`);
    });
